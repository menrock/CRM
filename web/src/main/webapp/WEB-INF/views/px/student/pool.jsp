...
<link rel="stylesheet" type="text/css" href="/_resources/easyui/themes/default/easyui.css">
	...
	function custAllocate(){
		...
		for(var i=1; i < selRows.length; i++){
			stuIds+=","+ selRows[i].id;
			/*	
			if( selRows[i].companyId != szCompanyId){
				$.messager.alert('提醒','分配资源的公司必须一致');
				return;
			}*/	
		}
		...
		if( szCompanyId != $('#assignUsersDialog').find("input[name=companyId]").val() ){
			//$('#assignUsersDialog').find("input[name=companyId]").val(szCompanyId);
			searchAssignUsers();
		}
		
		$('#assignUsersDialog').dialog('open');
	}
	...
	function doAllocate(){
		var params={};
		params.stuIds = $("#assignUsersDialog").find("input[name=stuIds]").val();
		params.zxgwId = $("#assignUsersDialog").find("select[name=gwId]").val();
		if( params.zxgwId == ''){
			alert('请选择顾问');
			return;
		}
