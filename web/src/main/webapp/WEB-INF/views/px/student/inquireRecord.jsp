  ...
  function jsSave() {
    ...
    function loadContactRecord(stuId){
    	...
    	$('#contactGrid').datagrid({
        	url: '/px/contactRecord/listByStu?stuId=' + stuId,        	
...
<table>
	...
	<tr>
		...
		<c:if test="${stu.id !=null && isZxgw}">
			<a href='javascript:void(0)' onclick="openUserDialog('share')" class="easyui-linkbutton" data-options="iconCls:'ext_group_add'">资源共享</a>
		</c:if>
	...
	<c:if test="${shareList !=null && shareList.size() >0}">
	<tr>
		<td></td>
		<td colspan='5'>
		<c:forEach var="item" items="${shareList}">
			<span>
			${item.fromUserName}共享给了${item.toUserName}
			<c:if test="${item.fromUserId == user.id}">
				<a href='javascript:void(0)' onclick="cancelShare(this,'${item.id}')" >撤回</a>
			</c:if>
			,</span>
		</c:forEach>
		</td>
...
<script type="text/javascript">
	...
	function userSelected(props){
		...
		if(szPurpose == 'share'){
			...
			$.ajax({
				type: 'POST',
				url: 'assign/allocate.do',
				data: {stuIds:'${stu.id}', zxgwId:props.id},
				dataType: "json",
				error:function(){ alert('系统异常'); },
				success: function(json){
					$('#zxgwGrid').datagrid('load');
