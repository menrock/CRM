...
<link rel="stylesheet" type="text/css" href="/_resources/easyui/themes/default/easyui.css">
	...
	$(function() {
		...
		$('#stu_level').combotree('loadData', [{
		   ...
		   <c:forEach var="item" items="${levelList}" varStatus="status" >  
		   		<c:if test="${status.index >0}">,</c:if>
		        {
		                ...
		                text: '<c:out value="${item.dictName}(${item.dictDesc})" />'
		        }
		   </c:forEach>  
		        ]
			}]);
	  ...
	  grid = $('#grid').datagrid({
				...
				columns : [ [ 
				{
					...
					title : '顾问回访时间',
					align:'left',
					field : 'gw_last_contact_date',
					sortable: true,
					formatter: function(value,row,index){
						if(!row.gwLastContactDate) return "";
					    var d = new Date(row.gwLastContactDate);
					...
					title : '顾问回访内容',
					align:'left',
					field : 'gwLastContacts',
					formatter: function(value,row,index){
						var records = row.gwLastContactRecords;
						if( !records )
						  return "";
					...
					title : '主管回访时间',
					align:'left',
					field : 'leader_last_contact_date',
					sortable: true,
					formatter: function(value,row,index){
						if(!row.leaderLastContactDate) return "";
					    var d = new Date(row.leaderLastContactDate);
					    return d.format('yyyy-MM-dd HH:mm'); 
					}
				},{
					width : 210,
					title : '主管回访内容',
					align:'left',
					field : 'leaderLastContacts',
					formatter: function(value,row,index){
						var records = row.leaderLastContactRecords;
						if( !records )
						  return "";
					...
					title : '客服回访时间',
					align:'left',
					field : 'kf_last_contact_date',
					sortable: true,
					formatter: function(value,row,index){
						if(!row.kfLastContactDate) return "";
					    var d = new Date(row.kfLastContactDate);
					    return d.format('yyyy-MM-dd HH:mm'); 
					}
				},{
					width : 210,
					title : '客服回访内容',
					align:'left',
					field : 'kfLastContacts',
					formatter: function(value,row,index){
						var records = row.kfLastContactRecords
						if( !records )
						  return "";
	...
	function selectUser(objName){
		...
		if( $('#divUsers').find('select[name=companyId]')[0].options.length	== 0){	
		...
		if( szCompanyId != $('#divUsers').find('select[name=companyId]').val() ){
			$('#divUsers').find('select[name=gwId]').empty();
			$('#divUsers').find('select[name=companyId]').val(szCompanyId);
			if( szCompanyId != ''){
				searchUsers();	
			}	
		}
	...
	function userSelected(){
		...
		if( $(sel).val() == ''){
			...
			$('#searchForm').find("select[name=assignFlag]").val('');
		}	
	...
	function custAllocate(){
		...
		var stuIds = selRows[0].id ;
		for(var i=1; i < selRows.length; i++){
			stuIds+=","+ selRows[i].id;
		}
		$('#assignUsersDialog').find("input[name=stuIds]").val(stuIds);
		
		$('#tblFromZxgw').hide();
		$('#tblToZxgw').show();
		
		var szCompanyId = selRows[0].companyId;
		if( szCompanyId != $('#assignUsersDialog').find("input[name=companyId]").val() ){
			searchAssignUsers();
		}
		
		$('#assignUsersDialog').dialog('open').dialog('center').dialog('setTitle','客户分配');
		$('#assignUsersDialog').find("input[name=actionType]").val("assign");
	}
	
	function searchAssignUsers(){
		var params = {'enabled':true};
		params.companyId = $('#assignUsersDialog').find("select[name=companyId]").val();
		params.name   = $('#assignUsersDialog').find("input[name=name]").val();
		params.online = $('#assignUsersDialog').find("select[name=online]").val();
		...
		$.ajax({
			type: 'POST',
		 	...
		 	error:function(){ alert('系统异常'); },
		 	success: function(json){
		 		$("#assignUsersDialog").find("select[name=gwId]").empty();
		 		$("#assignUsersDialog").find("select[name=gwId]").append("<option value=''>请选择</option>" ); 

		 		for(i=0; i < json.rows.length; i++){
		 			
		 			s = json.rows[i].id + "'>" + (json.rows[i].online?'[在线]':'[离线]') + json.rows[i].name + "</option>";
		 			$("#assignUsersDialog").find("select[name=gwId]").append("<option value='" + s ); 
		 		}
		 	}
		});
	}
	...
	function doAllocate(){
		var params={};
		params.stuIds     = $("#assignUsersDialog").find("[name=stuIds]").val();
		params.toZxgwId   = $("#assignUsersDialog").find("[name=gwId]").val();
		params.fromZxgwId = $("#assignUsersDialog").find("[name=fromZxgwId]").val();
		
		var szUrl = "/lx/custAssign/allocate.do";
		
		if( params.zxgwId == ''){
			$.messager.alert( {title: 'Error',msg:'请选择顾问'} ); 
			return;
		}
		
		if( !confirm("确认分配?")) return;
		szUrl = "/lx/custAssign/allocate.do";
		...
		$.ajax({
		  ...
		  success: function(json){
		  	...
		  	$('#assignUsersDialog').dialog('close');
		  }
		});
	}	
	
	function assignFlagChanged(obj){
		if(obj.value == 'false'){
			$('#searchForm').find("input[name=zxgw_id]").val( '' );
			$('#searchForm').find("input[name=zxgw_name]").val( '' );
		}	
	}
</script>	
</head>
...
<table width=800px>
  ...
  <tr><td width=100% align='center' height=30 class='title'>留学客户池</td></tr>
</table>
</c:if>
	...
	<table width="100%" border="0">
		<tr> 
			...
			<td style="width: 80px;text-align:right">分配状态</td>
			<td>
				<select name='assignFlag' onclick="assignFlagChanged(this)">
...
<div id='assignUsersDialog' class="easyui-dialog" title="选择顾问" data-options="closed:true,buttons:[{
				text:'确定',
				handler:doAllocate
			},{
				text:'关闭',
				handler:function(){$('#assignUsersDialog').dialog('close');}
			}]" style="width:420px;height:280px;padding:10px">
	<table id='tblToZxgw'>
		<tr>
			<td style='width:60px;font-weight:bold'>新分配顾问</td>
			<td></td>
		</tr>
		<tr>
			<td style='text-align:right'>公司</td>
			<td>
			<select name='companyId' onchange="searchAssignUsers()">
			<option  value=''>请选择</option>
			<c:forEach var="item" items="${companyList}">
			<option value='${item.id}' >${item.name}</option>
			</c:forEach>
		...
		<tr>
			...
			<td><input type='text' name='name' style='width:80px' />
				在线
				<select name='online' onchange="searchAssignUsers()">
					<option value=''>不限</option>
					<option value='true'>在线</option>
					<option value='false'>离线</option>
				</select>
			<a href='javascript:void(0)' onclick='searchAssignUsers()'>查询</a>
			</td>
		</tr>	
		<tr>
			<td style='text-align:right'>顾问</td>
			<td>
				<select name='gwId' style="width:120px;height:20px"></select>
