...
<link rel="stylesheet" type="text/css" href="/_resources/easyui/themes/extIcon.css">
	...
	$(function() {
	  ...
	  grid = $('#grid').datagrid({
				...
				columns : [ [ 
				{
					...
					title : '意向国家',
					halign:'center',
					field : 'a.plan_country',
					sortable: true,
					formatter: function(value,row,index){return row.student.planCountryName; } 
				},{
					width : 80,
					title : '申请学历',
					halign:'center',
	...
	function custTranAssign(){	
		var selRows = $('#grid').datagrid("getSelections");
		if( null == selRows || 0 == selRows.length){
			alert('请选择要操作的行');
			return;
		}
		var stuZxgwIds = selRows[0].id ;
		for(var i=1; i < selRows.length; i++){
			stuZxgwIds+=","+ selRows[i].id;
		}
		$('#assignUsersDialog').find("input[name=stuZxgwIds]").val(stuZxgwIds);
		...
		$('#assignUsersDialog').find("input[name=stuZxgwIds]").val(stuZxgwIds);
		$('#assignUsersDialog').dialog('open').dialog('center').dialog('setTitle','客户转分配');
		$('#assignUsersDialog').find("input[name=actionType]").val("tranAssign");
	}
	...
	function custRecycle(){
		...
		var params={};
		params.stuZxgwIds   = stuZxgwIds;
		
		var szUrl = "/lx/custAssign/revoke.do";
	...
	function doAllocate(){
		var params={};
		params.stuZxgwIds   = $("#assignUsersDialog").find("[name=stuZxgwIds]").val();
		params.toZxgwId   = $("#assignUsersDialog").find("[name=gwId]").val();
		if( params.toZxgwId == '' ){
			$.messager.alert('提醒','请选择新顾问');
			return;
		}
		
		var szUrl = "/lx/custAssign/tranAssign.do";
...
<table width=800px>
  ...
  <tr><td width=100% align='center' height=30 class='title'>资源管理</td></tr>
	...
	<form name="searchForm" id="searchForm" method="post" action="exportStuZxgw.do">
	<table width="100%" border="0">
		<tr> 
			...
			<td style="width: 200px">
				<input type='text' name='unitName' readonly style="width:150px" onclick="selectUnit()">
		...
		<tr>
			...
			<td colspan='7'>
				<select id="stu_level" name="stu_level" class="easyui-combotree" panelWidth="460" style="width:430px;" multiple="multiple"></select>
				&nbsp;
			...
			<c:if test="${canExport}">	
				...
				<a href="javascript:;" onclick="custTranAssign()" class="easyui-linkbutton" data-options="iconCls:'icon-crm-allocate'" 
				style="margin-left: 0px;" >转分配</a> 
	...
	<table id='tblToZxgw'>
		...
		<tr>
			...
			<td>
				...
				<input type='hidden' name='stuZxgwIds'>
				<input type='hidden' name='stuIds'>
