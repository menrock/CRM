...
<script language='javascript'>	
	...
	$(function() {			
	  grid = $('#grid').datagrid({
				title : '',
				url : 'payments4ConfirmData.do',
				queryParams:{confirmNode : '${confirmNode}'},
				striped:true,
				rownumbers : true,
				pagination : true,
				pageSize : 15,
	...
	function edit(id){
		$.ajax({
            ...
            success: function(result){
            	...
            	var buttons = null;
            	if( confirmNode == 'FZ_CONFIRM'){
            		if( result.data.status == 'SUBMIT'){
            			buttons = [
            				{text:'确认',handler:confirmPayment},
            				{text:'关闭',	handler:function(){$('#divPayment').dialog('close');}
            			}];
            		}else if( result.data.status == 'FZ_CONFIRMED'){
            			buttons = [
            		...
            		if( result.data.status == 'FZ_CONFIRMED'){
            			buttons = [
            				{text:'确认',handler:confirmPayment},
            				{text:'关闭',	handler:function(){$('#divPayment').dialog('close');}
            			}];
            	...
            	$('#divPayment').dialog({buttons:buttons});
	...
	function revokePayment(){ 
		var params =  $.serializeObject($('#paymentForm'));
		if( params.status != 'FZ_CONFIRMED' && params.status != 'CONFIRMED'){
	...
	<table style="width:800px;margin-top:10px">
		<tr>
			<td width=100% align='center' class='title'>
			  <c:if test="${ confirmNode == 'FZ_CONFIRM'}"> 分总收款确认</c:if>
			  <c:if test="${ confirmNode == 'CONFIRM'}"> 财务收款确认</c:if>
			</td>
		</tr>
	</table>
	...
	<table width="100%" border="0">
		<tr> 
			...
			<td >
				...
				<input type='hidden' name='confirmNode' value='${confirmNode}'/>
			</td>
		</tr>
		<tr>
